...
class NewAsset(object):
    ...
    def add_to_new_assets_zone(self):
        ...
        #如果数据库内没有该数据，那么新增，如果有，则更新，这就大大减少了我们的代码量，不用写两个方法。该方法的参数必须为一些用于查询的指定字段（这里是sn），以及需要新增或者更新的defaults字典。而其返回值，则是一个查询对象和是否新建对象布尔值的二元元组。
...
class UpdateAsset:
    ...
    def __init__(self, request, asset, report_data):
        ...
        self.asset.m_time = datetime.datetime.now()
        self.asset_update()
